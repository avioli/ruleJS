...
var Matrix = function (matrixDelegate) {
  ...
  this.updateItem = function (item, props) {
    ...
    if (item && props) {
      ...
      _.forEach(keys, function (key) {
        if (debug) {
          if (key === 'id') {
            debug && console.warn('Changing ID of Item!');
          }
        }
  ...
  this.addItem = function (item) {
    ...
    if (! existingCell) {
      ...
      // TODO: get rid of this - this function should only add items, not update them
      // debug && console.error('addItem: ' + item.id + ' item exists -- updating');
