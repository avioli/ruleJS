...
module.exports.core = function (dataSource) {
  ...
  var parse = function (formula, item) {
    ...
    // fml = 'IFERROR(SUM(C358:F358)/SUM(C257:F257,C263:F263),0)';
    // fml = 'SUM(C358:F358)';
    // fml = 'IFERROR(((J193+J188)/SUM(J203:M203))*J203,0)';
    if (formula === fml) {
      // if (debug) {
        window.marker = true;
      // }
    ...
    try {
      ...
      if (_.isString(result) && result.substr(0, 5) === 'Error') {
        console.error('parse', item, result);
      }

      if (window.marker) {
        _.forEach(item.deps, function(id) {
          var depItem = matrix.getItem(id);
          console.log(depItem);
        });
      }
      ...
      debug && window.marker && console.error('deps:');
      debug && window.marker && console.error(deps);
      ...
      if (deps.indexOf(id) !== -1) {
        ...
        _.forEach(deps, function (id) {
          matrix.updateItemValue(id, null, Exception.get('REF'));
        });
    ...
    debug && window.marker && console.log('parse', ret);
    debug && delete window.marker;
  ...
  var resultItem = function (item) {
    ...
    return {
      ...
      error: 'undefined' === typeof error ? null : error,
  ...
  var setItem = function (id, newValue) {
    ...
    if (utils.isFormula(newValue)) {
      var formula = newValue.substr(1);
      matrix.updateItemFormula(item, formula);
    } else {
      matrix.updateItemValue(item, newValue);
    }

    // CHECK: recalculate only the dependant items + time ones (and their dependant ones)
    if (matrix.bulkSetMode) {
      return false;
    } else {
      matrix.recalculate();
    }
  ...
  var recalculate = function () {
    matrix.recalculate(1);
  };

  /**
   * Recalculates the matrix after a bulk set callback
   */
  var recalculateAfterBulkSet = function (callback) {
    if (matrix.bulkSetMode) {
      throw new Error('Already in bulkSetMode');
    }

    matrix.bulkSetMode = true;

    callback();

    matrix.bulkSetMode = false;

    matrix.recalculate(); // for some reason sometimes just one recalculate isn't enough... dunno why
    matrix.recalculate();
    matrix.recalculate();
  }
  ...
  return {
    ...
    getCell: getItem,
    // setCell: setItem, // setting the formula is not implemented, yet
    setCellValue: setItemValue,
    getCells: getItems,
    getDependantIds: getDependantIds,
    recalculate: recalculate,
    recalculateAfterBulkSet: recalculateAfterBulkSet,
    error: error,
    isError: FormulaJS.ISERROR,
  };
